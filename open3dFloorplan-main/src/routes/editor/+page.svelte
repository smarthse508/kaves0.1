<script lang="ts">
  ...
  onMount(() => {
    (async () => {
      ...
      const ruangkerjaId = url.searchParams.get('ruangkerja_id');
      const withRuangkerja = (projectId: string) => `/editor?id=${projectId}${ruangkerjaId ? `&ruangkerja_id=${ruangkerjaId}` : ''}`;
      ...
      if (id) {
        ...
        if (project) {
          ...
          history.replaceState(null, '', withRuangkerja(p.id));
        }
      } else {
        const p = createDefaultProject();
        currentProject.set(p);
