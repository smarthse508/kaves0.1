<script lang="ts">
  ...
  let user = $state(null); // 🔥 Tambahkan state user
  let loadingAuth = $state(true); // 🔥 Tambahkan state loading
  ...
  const getRuangKerjaId = () => {
    const params = new URLSearchParams(window.location.search);
    return params.get('ruangkerja_id');
  };

  const editorUrl = (projectId: string) => {
    const ruangkerjaId = getRuangKerjaId();
    return ruangkerjaId
      ? `/editor?id=${projectId}&ruangkerja_id=${ruangkerjaId}`
      : `/editor?id=${projectId}`;
  };

  onMount(async () => {
    try {
      // 1. Cek sesi login ke backend (menggunakan cookie dari React)
      const authRes = await fetch(`${API_BASE}/api/user/me`, {
        credentials: 'include'
      });
      const authData = await authRes.json();

      if (authData.success) {
        user = authData.user; // Simpan data user jika login berhasil

        // 2. Jika sudah login, baru ambil list projects
        const data = await localStore.list();
        console.log("PROJECTS RAW:", data);
        projects = Array.isArray(data) ? data : [];

        // Sisanya (sorting & thumbnail) tetap sama
        projects.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
        const thumbs: Record<string, string | null> = {};
        await Promise.all(projects.map(async (p) => {
          thumbs[p.id] = await localStore.getThumbnail(p.id);
        }));
        thumbnails = thumbs;
      } else {
      ...
      console.error("Gagal mengecek auth:", err);
    } finally {
      loadingAuth = false;
    }
...
{#if loadingAuth}
  <div class="flex h-screen items-center justify-center bg-gray-50">
    <div class="text-center">
      <div class="mb-4 h-12 w-12 animate-spin rounded-full border-4 border-blue-500 border-t-transparent mx-auto"></div>
      <p class="text-gray-600 font-medium">Menghubungkan ke KAVES...</p>
    </div>
  </div>

{:else if !user}
  <div class="flex h-screen items-center justify-center bg-gray-100 p-6">
    <div class="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 text-center border border-red-100">
      <div class="text-5xl mb-4">🔒</div>
      <h1 class="text-2xl font-bold text-gray-800 mb-2">Akses Dibatasi</h1>
      <p class="text-gray-500 mb-6">
        Kamu belum login atau sesi telah berakhir. Silakan login kembali melalui aplikasi <strong>KAVES Utama</strong>.
      </p>
      <button
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {#each projects as project}
          <div class="group bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg hover:border-gray-300 transition-all duration-200 relative">
            <!-- Thumbnail -->
            <a href={editorUrl(project.id)} class="block">
